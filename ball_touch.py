...
def ball_state(frame,ball_boxes, player_boxes):
    ...
    if (len(ball_boxes) != 0): #检测到球
        ...
        if first_ball == 1 and (
                old_box_ball[:, 0] < loss_ball_edge or old_box_ball[:, 2] > frame.shape[1] - loss_ball_edge \
                or old_box_ball[:, 1] < loss_ball_edge or old_box_ball[:, 3] > frame.shape[0] - loss_ball_edge):
            state = 3 #球出边界所以没识别到
            print('ball out edge')
        elif first_ball == 0:
            state = 4 #还未第一次检测到球
