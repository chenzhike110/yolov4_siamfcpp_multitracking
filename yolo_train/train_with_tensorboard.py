...
if __name__ == "__main__":
    ...
    #   权值文件请看README，百度网盘下载
    #------------------------------------------------------#
    model_path = "logs/Epoch43-Total_Loss14.0681-Val_Loss14.9689.pth"#"model_data/yolo4_weights.pth"
    ...
    if False:
        ...
        if Use_Data_Loader:
            ...
            gen_val = DataLoader(val_dataset, shuffle=True, batch_size=Batch_size, num_workers=6,pin_memory=True, 
                                    drop_last=True, collate_fn=yolo_dataset_collate)
        else:
    ...
    if True:
        ...
        if Use_Data_Loader:
            train_dataset = YoloDataset(lines[:num_train], (input_shape[0], input_shape[1]), mosaic=mosaic, is_train=True)
            val_dataset = YoloDataset(lines[num_train:], (input_shape[0], input_shape[1]), mosaic=False, is_train=False)
            gen = DataLoader(train_dataset, shuffle=True, batch_size=Batch_size, num_workers=10, pin_memory=True,
                                    drop_last=True, collate_fn=yolo_dataset_collate)
            gen_val = DataLoader(val_dataset, shuffle=True, batch_size=Batch_size, num_workers=10,pin_memory=True, 
                                    drop_last=True, collate_fn=yolo_dataset_collate)
        else:
            gen = Generator(Batch_size, lines[:num_train],
                            (input_shape[0], input_shape[1])).generate(train=True, mosaic = mosaic)
